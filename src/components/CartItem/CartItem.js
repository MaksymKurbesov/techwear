...
const CartItem = ({ product, size, removeItem, changeQuantity }) => {
  ...
  return ( // если элемент удалён, произвести анимацию 
    <tr className={isDeleted ? styles.deleted : ''}>  
      <td>
        <img src={product.img} alt={product.model} title={product.model} width="100" height="100"/>
      </td>
      <td>
        {product.brand}
      </td>
      ...
      <td>
        <CartItemQuantityInput quantity={product.quantity} changeQuantity={handleChangeQuantity}/>
      ...
      <td>
        <button className={styles.deleteItemButton} onClick={() => {
          setIsDeleted(true)
          setTimeout(() => {
